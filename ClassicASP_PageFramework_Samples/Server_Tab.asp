	...
	Class ServerTab
		...
		Public Function RenderCaption(Width,ColSpan,IsVisible)
			...
			If Owner.AutoPostBack Then
				Response.Write Page.GetEventScript("onclick", Owner.Control.Name, "TabClick", TabIndex,"")
			Else
				Response.Write " onclick = ""__ShowTab(this,'" & Owner.Control.Name & "')"" "
			End If
	...
	End Class
	 ...
	 Class ServerTabStrip		
	    ...
	    End Sub
	   	...
	   	Public Function ReadProperties(bag)				   			   		
			If Not AutoPostBack Then
				 SelectedTabIndex = CInt(Request.Form(Control.Name & "_I"))
			Else
		...
		Private Function RenderTab()
			...
			'keep track of it...
			If Not AutoPostBack Then
				Response.Write "<INPUT TYPE=HIDDEN ID='" & Control.Name & "_I' NAME='" & Control.Name & "_I' Value='" & SelectedTabIndex & "'>"
			End If
		End Function
